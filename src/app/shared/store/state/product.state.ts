...
export class ProductState {
  ...
  getProducts(ctx: StateContext<ProductStateModel>, action: GetProductsAction) {
    return this.productService.list(action.payload).pipe(
      tap({
        next: (result: any) => {
          // Hem {data:[...]} hem de doğrudan [...] için güvenli parse
          const items = Array.isArray(result) ? result : (result?.data ?? []);
          let paginateProduct;
          
          if (action.payload!['page'] && action.payload!['paginate']) {
            paginateProduct = items
              .map(product => ({ ...product }))
              .slice(
          ...
          if (action?.payload!['top_selling']) {
            ...
            ctx.patchState({
              product: {
                ...
                total: result?.meta?.total ?? result?.total ?? paginateProduct?.length,
              },
            });
          }
        },
  ...
  updateStatus(
    ...
    { id, status }: UpdateProductStatusAction,
  ) {
    return this.productService.updateStatus(id, status).pipe(
      tap({
        next: () => {
          const state = ctx.getState();
          const updatedProducts = state.product.data.map(product => 
            product.id === id ? { ...product, is_active: status } : product
          );
          ctx.patchState({
            ...state,
            product: {
          ...
          this.notificationService.showSuccess('Ürün durumu başarıyla güncellendi');
        },
        error: err => {
          this.notificationService.showError('Ürün durumu güncellenirken hata oluştu');
          throw new Error(err?.error?.message);
        }
      })
    );
